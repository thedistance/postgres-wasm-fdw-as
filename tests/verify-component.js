// Script to verify that the WebAssembly module is compatible with the Supabase Wrappers framework
...
const COMPONENT_PATH = join(rootDir, 'build-component', 'as_wasm_fdw.wasm');
...
async function verifyComponent() {
  try {
    console.log(`Loading component WebAssembly module from ${COMPONENT_PATH}...`);
    ...
    if (allFunctionsPresent) {
      ...
      console.log('✅ The WebAssembly module is compatible with the Supabase Wrappers framework.');
    } else {
      console.log('❌ Some required functions are missing.');
      console.log('❌ The WebAssembly module is NOT compatible with the Supabase Wrappers framework.');
    }
